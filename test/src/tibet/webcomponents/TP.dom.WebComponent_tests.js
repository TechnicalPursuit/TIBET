...
TP.dom.WebComponent.Type.describe('TP.dom.WebComponent: anonymous webcomponent - no shadow DOM',
function() {
...
function() {
    ...
    this.before(
        async function(suite, options) {
            ...
            loadURI = TP.uc('~lib_test/src/tibet/webcomponents/test_templateexpander.xhtml');
            await driver.setLocation(loadURI);
        });
    ...
    this.it('Rendering', async function(test, options) {

        var resultSpans;

        //  Change the content via 'user' interaction

        resultSpans = TP.byCSSPath('span.result', windowContext, true);
        test.assert.isSizeOf(resultSpans, 2);
    });
...
TP.dom.WebComponent.Type.describe('TP.dom.WebComponent: customized built-in webcomponent - no shadow DOM',
function() {
    ...
    this.before(
        async function(suite, options) {
            ...
            loadURI = TP.uc('~lib_test/src/tibet/webcomponents/test_expandinglist.xhtml');
            await driver.setLocation(loadURI);
    ...
    this.it('Activation - mouse', async function(test, options) {
        ...
        ukTrigger = TP.byCSSPath('#UKTrigger > span', windowContext, true);
        ukList = TP.byId('UKList', windowContext);

        usTrigger = TP.byCSSPath('#USTrigger > span', windowContext, true);
        usList = TP.byId('USList', windowContext);

        test.refute.isVisible(ukList);
        test.refute.isVisible(usList);
...
TP.dom.WebComponent.Type.describe('TP.dom.WebComponent: lifecycle',
function() {
    ...
    this.before(
        async function(suite, options) {
            ...
            loadURI = TP.uc('~lib_test/src/tibet/webcomponents/test_customsquare.xhtml');
            await driver.setLocation(loadURI);
        });
    ...
    this.it('Activation - mouse', async function(test, options) {
        ...
        addButton = TP.byCSSPath('button.add', windowContext, true);
        updateButton = TP.byCSSPath('button.update', windowContext, true);
        removeButton = TP.byCSSPath('button.remove', windowContext, true);

        await driver.constructSequence().
            click(addButton).
            run();

        customSquare = TP.byCSSPath('custom-square', windowContext, true);
        test.assert.isVisible(customSquare);

        test.assert.isEqualTo(
            customSquare.getAttribute('l'),
            '100');
        test.assert.isEqualTo(
            customSquare.getAttribute('c'),
            'red');
        ...
        customSquare = TP.byCSSPath('custom-square', windowContext, true);
        test.assert.isEmpty(customSquare);
    });
