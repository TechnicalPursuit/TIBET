	...
	*/
	...
	while (c) {
		...
		if ((c <= ' ') || (c.charCodeAt(0) === 160)) {
			...
			//	a special case here to help support TSH scripts during
			//	tokenization is to check for '${' as an operator before
            //	presuming that '$' is a single-character identifier
            isSpecialSub = (str === '$' && src.charAt(i) === '{');

			if (tsh && isSpecialSub) {

				//	${variable} is a special form of substitution for the
				//	TIBET Shell so we treat it similar to quoted strings
				//	when we find it outside of a string. When found inside a
				//	string the TIBET interpolate call processes it directly.

                //  'str' already had the '$', so add '{' onto it and
                //  advance 'i'
                str += '{';
                i += 1;
				...
				for (;;) {
					c = src.charAt(i);
					if (c === '') {
						TP.boot.$stderr('Unterminated substitution: ' +
					...
					if (c === '}') {
						//	bit tricky since you can have multiple \'s and
						//	we need to know if the number is odd or even to
...
TP.$tokenizeTemplate = function(src) {

	/**
	@method		$tokenizeTemplate
	@abstract	Processes a string into tokens based on TIBET templating
				syntax which consists of ${template} or ${{nested template}}
				constructs. NOTE that unlike the $tokenize function this
				function is not concerned with anything other than escape
				sequences for the leading $ which allow templates to
				generate other templates. No attention is given to strings,
				regular expressions, or other "quoting" in JavaScript.
	@param		src			String		The JavaScript source to process.
	@return		Array		An array of token objects, simple objects which
							represent one token within the source string.
	*/
	...
	while (c) {
		...
		if ((c === '$') && (src.charAt(i + 1) === '{')) {
			...
			//	odd? escaped
			if (count % 2 !== 0) {
				//	this actually removes the \ from the template content
				//	since once it acts to preserve the $ its work is done
				str = str.slice(0, str.length - 1);
				str += String.fromCharCode(src.charCodeAt(i));
			...
			//	any string we've been working on should now be pushed
			//	as a token so we can start collecting substitution content
			if (str) {
				result.push(new_token('string', str));
			}

			//	start the substitution token's text value
			str = '${';
			i += 2;
			block = 0;

			//	scan until we find unescaped }'s to balance out opening
			for (;;) {
				c = src.charAt(i);
				if (c === '') {
					//	bad, ran out of chars before finding }[}]
					TP.boot.$stderr('Unbalanced template substitution: ' +
								TP.boot.$dump(new_token('string', str),
								', '));
					break;
				}
				...
				//	watch for unescaped nesting, which increases our block
				//	count. we won't close off the substitution until the
				//	nesting level is 0.
				//if ((c === '$') && (src.charAt(i + 1) === '{'))
				if (c === '{') {
					j = i - 1;
					...
					//	odd? escaped
					if (count % 2 !== 0) {
						//	this actually removes the \ from the template
						//	content since once it acts to preserve the $ its
						//	work is done, and we don't increment the block
						//	count so we don't see this as nested
						str = str.slice(0, str.length - 1);
					} else {
						//	not escaped...increment nesting level
						block++;
					}
				...
				//	if we've got a } we need to know it's not escaped...and
				//	if it isn't then we've got to handle nesting level.
				if (c === '}')
				{
					j = i - 1;
					count = 0;
					...
					//	odd? escaped
					if (count % 2 !== 0) {
						//	escaped...keep moving after removing the escape
						str = str.slice(0, str.length - 1);
						str += String.fromCharCode(src.charCodeAt(i));
	...
	//	anything left over at the end we push as a simple string 
	if (str) {
		result.push(new_token('string', str));
	}
...
TP.$condenseJS = function(src, newlines, spaces, operators, tokens, nojoin,
	...
	*/
	...
	arr = TP.$tokenize(src, operators, tsh);
	len = arr.length;
...
TP.$expandJS = function(src) {

	/**
	@method		$expandJS
	@abstract	Expands a condensed string of JavaScript, reinjecting
				newline characters after ; operators to help identify where
				a missing semicolon may be in the original source.
	@param		src				String		The JavaScript source to
											process.
	@return		String|Array	The expanded source string.
	*/
	...
	arr = TP.$tokenize(src, null, null, true);
	len = arr.length;
