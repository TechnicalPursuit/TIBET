...
//  simple object for looking up reusable notification windows
...
TP.boot.$notDefined = function(anObj) {

    /**
     * @name $notDefined
     * @synopsis Returns true if the value provided is undefined.
     * @param {Object} anObj The object to test.
     * @return {Boolean} Whether or not anObj is undefined.
     */
...
TP.boot.$notEmpty = function(aValue) {

    /**
     * @name $notEmpty
     * @synopsis Returns true if the object provided is valid and not either the
     *     empty string or an empty collection.
     * @param {Object} aValue The object to test.
     * @return {Boolean}
     */

    return !TP.boot.$isEmpty(aValue);
};

//  ------------------------------------------------------------------------

TP.boot.$notTrue = function(aValue) {
...
TP.boot.$owns = function(anObj, aName) {
     ...
     * @return {Boolean} True if the object owns the slot.
     * @todo
     */

    try {
        return anObj.hasOwnProperty(aName);
    } catch (e) {
        try {
            //  MOZ has a few that like to barf on this call. Unfortunately,
            //  false isn't always the right answer :(
            return anObj[aName] !==
                            anObj.constructor.prototype[aName];
        } catch (e2) {
            return false;
        }
    }
...
if (window !== top &&
    ...
    window.frameElement.hasAttribute('tibet:settinglocation') !== true)
{
    ...
    TP.boot.$formatCookieValue = function(anObj) {

        /**
         * @name $formatCookieValue
         * @synopsis Formats an object into cookie format. The format used in
         *     this version is a trivial string representation. Unlike the
         *     TP.core.Cookie type in TIBET you can't count on the result being
         *     valid JS source code, but it usually is.
         * @param {Object} anObj The object to format as a cookie.
        ...
        if (anObj.constructor === Array) {
            str = '[';
            for (i = 0; i < anObj.length; i++) {
                str += (i > 0) ? ',' : '';
                str += TP.boot.$formatCookieSource(anObj[i]);
            }
            str += ']';
        ...
        if (anObj.constructor === Object) {
            str = '{';
            cnt = 0;
            for (i in anObj) {
                str += (cnt > 0) ? ',' : '';
                str += '\'' + i + '\' : ' +
                        TP.boot.$formatCookieSource(anObj[i]);
                cnt++;
            }
            str += '}';
        ...
        return TP.boot.$formatCookieSource(anObj);
    };
    ...
    TP.boot.removeCookie = function(aName, aPath, aDomain) {

        /**
         * @name removeCookie
         * @synopsis Deletes the named cookie.
        ...
        d = (new Date()).getTime();
        d = new Date(d - (1000 * 60 * 60 * 24 * 365));

        TP.boot.setCookie(aName, '', d, aPath, aDomain);

        return TP.boot.getCookie(aName);
    };
    ...
    */
    ...
    TP.boot.$nextelem = function(anID) {

        /**
         * @name $nextelem
         * @synopsis Returns the nextSibling element of the element whose ID is
         *     anID. This is a convenience wrapper used primarily by the
         *     prefs.html page to manage labels related to UI controls.
         * @param {String|Element} anID The string ID to locate, or an element
         *     (which will be returned).
         * @return {Element}
         */
        ...
        elem = typeof(anID) === 'string' ? document.getElementById(anID) : anID;
    ...
    TP.boot.clearUICanvas = function() {

        /**
         * @name clearUICanvas
         * @synopsis Clear's the uicanvas's 'page' (that is, it empties its
         *     'body' element of the current content).
         * @return {null}
         */
        ...
        if (TP.boot.$isValid(win)) {
            TP.nodeEmptyContent(win.document.body);
        }
    ...
    TP.boot.showBootError = function() {

        /**
         * @name showBootError
         * @synopsis Sets the content of the UI frame to the bugs content DIV.
         * @return {null}
         */
    ...
    TP.boot.showLogin = function() {

        /**
         * @name showLogin
         * @synopsis Sets the content of the UI frame to the login.html page.
         * @return {null}
         */

        TP.boot.showUICanvas('~app_html/login.html');
    ...
    TP.boot.showPreferences = function() {

        /**
         * @name showPreferences
         * @synopsis Sets the content of the UI frame to the prefs.html page.
         * @return {null}
         */

        TP.boot.showUICanvas('~app_html/prefs.html');
    ...
    TP.boot.showUnsupported = function() {

        /**
         * @name showUnsupported
         * @synopsis Sets the content of the UI frame to the 'unsupported'
         *     content DIV when the tibet_boot script detects an unsupported
         *     browser.
         * @return {null}
         */
        ...
        TP.boot.showContent('unsupported');
    ...
    TP.boot.toggleTools = function() {

        /**
         * @name toggleTools
         * @synopsis Toggles the 'developer tools' if they are installed.
         * @return {null}
         */
        ...
        tWin = $$findTIBET();
        if (TP.boot.$isValid(tWin)) {
            //  try to find a reference to the TDP (TIBET Developer Portal). If
            //  its installed, it will be registered under this ID.
            tdp = tWin.$byOID('DeveloperPortal');
            if (TP.boot.$isValid(tdp)) {
                //  Toggle the TDP.
                tdp.toggleOpen();
            }
        }
    ...
    */
    ...
    if (TP.boot.$notValid(TP.DOM_SIGNAL_TYPE_MAP)) {
        //  NOTE that this is duplicated in the kernel DOM primitives so if you
    ...
    if (TP.boot.$notValid(atob)) {
        atob = function(aString) {
            /**
             * @name atob
             * @synopsis An implementation of the atob function found in Mozilla
             *     which takes a Base64-encoded ascii string and decodes it to
             *     binary form.
    ...
    */
    ...
    //  supplying the offset* properties for custom elements. We rectify that
    //  here.
    if (TP.boot.$$isMoz()) {
    ...
    if (TP.boot.$$isMoz() || TP.boot.$$isWebkit()) {
        Event.prototype.__defineGetter__(
            ...
            function() {
                ...
                //  The spec says that offsetX should be computed from the left
                //  of the *padding box* (i.e. including the padding, but
                ...
                if (target && target.nodeType !== Node.DOCUMENT_NODE) {
                    ...
                    if (compStyle.position === 'absolute' ||
                        ...
                        absOffset = TP.elementGetOffsetFromContainer(target);
        ...
        Event.prototype.__defineGetter__(
            ...
            function() {
                ...
                //  The spec says that offsetY should be computed from the top
                //  of the *padding box* (i.e. including the padding, but
                //  excluding the border).
        ...
        Event.prototype.__defineGetter__(
            ...
            function() {
                ...
                if (TP.boot.$isValid(resolvedTarget = this._resolvedTarget)) {
        ...
        Object.defineProperty(
            ...
            'currentTarget',
            {
                get: function() {
                    ...
                    //  We can never compute 'currentTarget' for IE in TIBET,
                    //  since we use document-level event handlers
                    return null;
                }
            });
    ...
    Event.prototype.at = function(key) {

        return this[key];
    };

    Event.prototype.atPut = function(key, value) {
    ...
    //  Add a 'copy' method for native Event objects that just returns a literal
    //  object.
    ...
    //  As of Firefox 22, 'NamedNodeMap' only deals with attributes (as per DOM
    //  Level 4). This alias patches that for us.
    if (TP.boot.$notValid(window.NamedNodeMap)) {
        /* global MozNamedAttrMap:false*/
        NamedNodeMap = MozNamedAttrMap;
        Object.defineProperty(MozNamedAttrMap, '$$name',
    ...
    */
    ...
    TP.boot.$$setupMetadata = function() {
        ...
        //  ** NOTE NOTE NOTE **
        //  We use 'TP.ac()' here throughout this method because these Arrays
        //  need to be created *as TIBETan enhanced Arrays*. They will be used
        //  for reflection purposes, etc. *even if this Window is not the TIBET
        //  window*. This is why we check not only for 'TP', but 'TP.ac' below.
    ...
    //  See the ** NOTE NOTE NOTE* above about why we check not only for 'TP'
    //  here, but for 'TP.ac' (we also check to see if the Object constructor
    //  has a '$$name' slot just to avoid doing this again if it's been done). 
    if (window.TP && window.TP.ac && !Object.$$name) {
    ...
    if (TP.boot.$$isIE()) {
        ...
        //  HTMLElement
        HTMLElement.prototype.getKeys = function() {

            return TP.sys.$htmlelemkeys;
        };
    }

    //  We do this for HTML Documents, but the prototype is different between
    //  W3C browsers and IE...
    if (TP.boot.$$isIE()) {
        Document.prototype.getKeys = function() {
        ...
        //  HTMLDocument
        HTMLDocument.prototype.getKeys = function() {
    ...
    Error.prototype.getKeys = function() {

        return TP.sys.$errorkeys;
    };

    Event.prototype.getKeys = function() {

        return TP.sys.$eventkeys;
    };

    NamedNodeMap.prototype.getKeys = function() {

        return TP.sys.$namednodemapkeys;
    };

    NodeList.prototype.getKeys = function() {
    ...
    Window.prototype.getKeys = function() {

        return TP.sys.$windowkeys;
    };

    XMLHttpRequest.prototype.getKeys = function() {
    ...
    */
    ...
    TP.boot.initializeCanvas = function() {
        ...
        */
        ...
        //  event notifications
        TP.core.Window.installLoadUnloadHooks(window);

        //  install basic TIBET feature set
        TP.core.Window.instrument(window);
        ...
        TP.boot.$$documentSetup(document);
    ...
    else if (TP.boot.getURLBookmark()) {
                ...
                TP.sys.cfg('boot.twophase') === true)
    {
        ...
        //  the kernel is loading "underneath us" in the code frame. Since this
        //  is all happening asynchronously we want to avoid issues caused by
        //  assuming any functionality is in place before we initialize the
        //  canvas.
        ...
        //  the trick is getting the sequencing right. essentially we may have
        //  placed the phase two page in place before or after the phase one
        //  components complete their load process. when we get there early we
        //  have to wait until the boot completes. and if it were to fail for
        //  any reason we have to eventually terminate our observation so that
        //  the browser doesn't sit there iterating until the end of time. if we
        //  arrive late then the main boot logic won't be active any longer and
        //  we'll have to "reawaken it" to get it to pick up with phase two.
        ...
        if (window.$$phasetwo === true) {
            if (TP.sys.cfg('boot.phasetwo') === true) {
                ...
                //  and we can be pretty sure that no matter where things are in
                //  the boot process we can start the initialization loop
                TP.boot.initializeCanvas();
            } else {
                //  to deal with the fact that the 'tibet' target may be in any
                //  stage of loading we'll create a function that either of the
                //  two sides can invoke to finish things
                TP.boot.bootPhaseTwo = function() {
                    ...
                    //  notify the main boot code logic that phase two should be
                    //  imported. we'll leave it up to that code to do the real
                    //  work :)
                ...
                //  if the boot is paused it's because we got here late, so it's
                //  up to us to trigger the final stage
                if (TP.boot.$$stage === 'import_paused') {
                    TP.boot.bootPhaseTwo();
                }
            }
        } else {
            //  we must be a phase one page in a two-phase world. which means we
            //  might sit here forever waiting for the user to get authenticated
            //  or otherwise trigger a phase-two page. might as well initialize.
            TP.boot.initializeCanvas();
        }
    }
    else    //  single phase or post-boot
    {
        ...
        //  since we know that all code will load in a single phase without any
        //  kind of user-dependent pause we can start up the initializeCanvas
        //  loop here
        TP.boot.initializeCanvas();
    }
