 ...
 * @synopsis XMLSchema simple type wrapper.
 */

//  ------------------------------------------------------------------------

TP.xs.XMLSchemaCompositeType.defineSubtype('simpleType');

//  ------------------------------------------------------------------------
//  Type Methods
//  ------------------------------------------------------------------------

TP.xs.simpleType.Type.defineMethod('defineTypesFromElements',
function(typeElems, forceDefinition) {
     ...
     * @returns {TP.lang.RootObject.<TP.xs.simpleType>} The receiver.
     * @todo
     */
    ...
    typeElems.perform(
        function(aSchemaElem) {
            ...
            if (TP.notEmpty(typeName =
                            TP.elementGetAttribute(aSchemaElem, 'name'))) {

                if (TP.isTrue(forceDefinition) ||
                    TP.notValid(type = TP.sys.require(typeName))) {

                    type = TP.xs.simpleType.defineSubtype(typeName);
...
TP.xs.simpleType.Type.defineMethod('validate',
function(aValue) {
...
TP.xs.simpleType.Type.defineMethod('validateList',
function(aValue, listElem) {
    ...
    if (TP.isEmpty(typeName)) {
        ...
        type = TP.xs.simpleType.defineSubtype(typeID);
...
TP.xs.simpleType.Type.defineMethod('validateRestriction',
...
TP.xs.simpleType.Type.defineMethod('validateUnion',
