...
//  A simple numeric counter used to track the load index of types, methods,
//  external objects, etc.
TP.__loadIndexCount__ = 0;
...
TP.registerLoadInfo = function(anObject) {
    ...
    if (!TP.ObjectProto.hasOwnProperty.call(anObject, TP.LOAD_INDEX)) {
        anObject[TP.LOAD_INDEX] = TP.__loadIndexCount__++;
    }
...
TP.sort.TABINDEX_ORDER = function(a, b) {
    ...
    //  document order.
    if (TP.isNaN(aVal) && TP.isNaN(bVal)) {
        return TP.sort.DOCUMENT_ORDER(a, b);
    } else if (TP.isNaN(aVal) && !TP.isNaN(bVal)) {
...
//  Note here how we make the space optional - it's not required and the code
//  might be minified.
...
TP.regex.ONLY_NEWLINE_WHITESPACE = /^[\r\n]+$/;
TP.regex.ONLY_NON_NEWLINE_WHITESPACE = /^[^\S\n]+$/;
