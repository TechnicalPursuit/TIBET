...
TP.lang.RootObject.Type.defineMethod('defineNativeClass',
function(name, classDefFunction) {

    /**
     * @method defineNativeClass
     * @summary Defines a new subtype of the receiver as an ECMAScript Class,
     *     Note that this has multiple limitations compared to TIBET types,
     *     including limited metadata about the class and the inability to
     *     define multiple inheritance (trait) relationships.
     * @description This method produces an Object that can act in the stead of
     ...
     *     namespace prefix and colon or dot separator.
     * @param {Function} classDefFunction The class definition Function that is
     *     created via an ECMAScript 6 'class expression'.
    ...
    //  name that was supplied. Note that we *cannot* do this by looking at the
    //  TP.global object or the TP object or whatever. The object that gets put
    //  into the global scope under the supplied name is *not* the TIBET-side
    //  type (which can be obtained using this mechanism) - it will be the Proxy
    //  that we create below that is a stand-in for the ECMAScript class.
    //  Therefore, unlike regular TIBET types, for this 'type'/'class' the
    //  following is *not* true:

    //  TP.MyNS.MyType === TP.sys.getTypeByName('TP.MyNS.MyType'); -> false

    subclassTIBETType = TP.sys.getTypeByName(name);

    //  If not, then define a TIBET-side type that will act as this class's
    //  stand-in on the TIBET-side of the house.
    if (!TP.isType(subclassTIBETType)) {
        subclassTIBETType = this.defineSubtype(name);
    } else {
        return;
    }

    //  Split the name that was supplied and use its parts to place a slot into
    //  the global space that will point to the proxy we're creating here.
    parts = name.split(/:|\./);
    switch (parts.length) {
        case 1:
            root = this.getNamespaceRoot();
    ...
    //  Define a Function that will copy over TIBET-related properties and
    //  install them on the TIBET-side type.
    copyTIBETProps = function(aTIBETType, sourceObj, track) {

        var propNames;

        //  Grab all of the class or instance (depending on what the target
        //  object is) properties off of the ECMAScript class and iterate.
        propNames = Object.getOwnPropertyNames(sourceObj);
        ...
        propNames.forEach(
            function(propName) {
                ...
                //  Grab the value - if it's a Function, add it as a method to
                //  the TIBET-side type. If it's not, add it as an attribute to
                //  the TIBET-side type.

                propVal = sourceObj[propName];

                if (TP.isFunction(propVal)) {
                    aTIBETType[track].defineMethod(
                        propName,
                        propVal);
                } else {
                    aTIBETType[track].defineAttribute(
                        propName,
                        propVal);
                }
            });
    ...
    proxyConfig = {
        get: function(target, propName) {
            ...
            //  If the propName is a Symbol, then just return whatever the
            //  TIBET-side type returns.
            if (typeof propName === 'symbol') {
                return subclassTIBETType[propName];
            }

            //  For 'constructor' or 'prototype', because these are considered
            //  'special', we return what the *target* returns returns.
            ...
            if (TP.notValid(subclassTIBETType.$$esClassInitialized)) {
                    ...
                    copyTIBETProps(subclassTIBETType,
                                    classDefFunction,
                                    TP.TYPE_TRACK);
                    copyTIBETProps(subclassTIBETType,
                                    classDefFunction.prototype,
                                    TP.INST_TRACK);
            ...
            //  For 'name', we return what the *target* returns.
            if (propName === 'name') {
                return subclassTIBETType[TP.NAME];
            }

            //  For everything else just return whatever the TIBET-side type
            //  returns.
            return subclassTIBETType[propName];
        },
        ...
        construct: function(target, args, newTarget) {
            ...
            //  Create an instance of the class by executing the target
            //  Function (which will execute the constructor as defined by class
            //  description Function) with the supplied args, but supply an
            //  optional constructor to use as the prototype of the resultant
    ...
    //  up the 'instance'-side of the chain and the second one wires up the
    ...
    //  don't care about the return value here, since we're not actually
    //  creating a real type entry here - we just need to make sure its present.
    TP.defineNamespace(root + '.' + nsName);

    //  Use the computed parts that were split out from the name that was
    //  supplied to place a slot into the global space that will point to the
    //  proxy we're creating here.
    TP.global[root][nsName][subtypeName] = classProxy;
...
function() {
    ...
    if (owner[TP.ID] === 'TP' || owner[TP.ID] === 'TP.sys') {
        ...
        obj[TP.LOAD_INDEX] = this[TP.LOAD_INDEX];
...
function(varargs) {
    ...
    //  If we have a C3 cache blow it away
...
function(propName, sources, track) {
    ...
    if (TP.isTrue(TP.sys.cfg('oo.$$traits_autoresolve'))) {
        ...
        //  Compute the C3 linearization for the target type. Note that without
        //  flags this returns a non-strict (TIBET precedence order) list of
        //  type names (not type instances).
        c3TypeList = this.getC3ResolutionOrder();
...
TP.lang.RootObject.Type.defineMethod('getC3ResolutionOrder',
function(strict, types) {
    ...
    //  From the wikipedia article one sample of this is...
    ...
    //  e.g. --  L(K3) := [K3] + merge(L(D), L(A), [D, A])
    list = list.concat(this.performC3Merge(this.getC3MergeData(strict)));
...
TP.lang.RootObject.Type.defineMethod('getC3MergeNames',
function(strict) {

    /**
     * @method getC3MergeNames
     * @summary Returns a list of the supertype names (supertype and traits)
     *     associated with the receiver. By default this method reverses the
     *     order so you get the list in "last overlay first" order which is how
     *     TIBET authoing represents resolution order will occur.
     * @param {Boolean} strict Set to true to use the strict C3 algorithm.
     * @returns {Array} The array of supertype/trait names.
     */

    var list;

    list = this.getC3MergeTypes(strict);
...
TP.lang.RootObject.Type.defineMethod('getC3MergeTypes',
function(strict) {

    /**
     * @method getC3MergeTypes
     * @summary Returns a list of the supertypes (supertype and trait types)
     *     associated with the receiver. By default this method reverses the
     *     order so you get the list in "last overlay first" order which is how
     *     TIBET authoing represents resolution order will occur.
     * @param {Boolean} strict Set to true to use the strict C3 algorithm.
     * @returns {Array} The array of supertype/trait types.
     */

    var sooper,
        list;

    sooper = this.getSupertype();

    //  Stop at the top!!!
    if (TP.notValid(sooper) || sooper === Object) {
        return [];
    }
    ...
    if (TP.owns(this, TP.TRAITS)) {
        list = list.concat(this[TP.TRAITS]);
    }

    //  If we're not doing strict C3 reverse so our last trait (what we think of
    //  as the most influential overlay) actually comes first.
    if (TP.isFalsey(strict)) {
        list.reverse();
    }
...
TP.lang.RootObject.Type.defineMethod('getC3MergeData',
function(strict) {

    /**
     * @method getC3MergeData
     * @summary Returns an array of arrays containing the C3 resolution orders
     *     for all supertype/traits of the receiver as well as the list of
     *     supertype/trait names being resolved. This structure is defined as
     *     the data provided to the "merge" function (performC3Merge).
    *       SEE https://en.wikipedia.org/wiki/C3_linearization
     * @param {Boolean} strict Set to true to use the strict C3 algorithm.
     * @returns {Array} The array of C3 resolutions and merge names.
     */
    ...
    //  e.g. [D, A]
    types = this.getC3MergeTypes(strict);

    //  e.g. -- L(D), L(A)
    list = types.map(function(type) {
        return type.getC3ResolutionOrder(strict);
    });

    //  plus [D.name, A.name]
    if (TP.isFalsey(strict)) {
        list.unshift(this.getC3MergeNames(strict));
    } else {
        list.push(this.getC3MergeNames(strict));
    }
...
TP.lang.RootObject.Type.defineMethod('performC3Merge',
function(c3MergeData) {

    /**
     * @method performC3Merge
     * @summary Returns an
     *
     *       SEE https://en.wikipedia.org/wiki/C3_linearization
     * @param {Array.<Array>} c3MergeData Merge data as produced by the
     *     getC3MergeData routine. This should be an array of arrays
     *     containing type names to be merged per the C3 algorithm.
     * @returns {Array} The array of merged (reduced) type names.
     */
    ...
    best = heads.detect(function(key) {
        return !tails.some(function(arr) {
    ...
    TP.ifDebug() ?
        TP.debug(this.getName() + ' c3 chose ' + best) : 0;
...
TP.defineMetaInstMethod('handleClearContentFromANYWhenANY',
function(aSignal) {
