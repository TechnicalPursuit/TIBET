...
async function(targetDoc, cssHref, inlineRuleText, shouldSignal) {
    ...
    if (TP.isTrue(inlineRuleText)) {
        ...
        newNativeElem = await TP.documentAddCSSLinkElement(
...
function(targetDoc, linkHref, beforeNode, shouldSignal, loadedHandler) {
     ...
     *     should be added.
     * @param {TP.uri.URI|String} linkHref The href to use on the newly added
     *     'link' element.
     ...
     *     loaded. This should take one parameter, the newly added link element.
     * @exception TP.sig.InvalidDocument
     * @returns {Promise} A Promise that will resolve with The new link element
     *     that was added.
     */
    ...
    if (TP.notValid(url)) {
        ...
        return Promise.reject(new Error('InvalidURI'));
    }
    ...
    newLinkElement[TP.GENERATED] = true;
    ...
    promise = Promise.construct(
        function(resolver, rejector) {
            ...
            successHandler = function(evt) {

                //  Start by removing the 'onload' and 'onerror' handlers from
                //  our link element.
                newLinkElement.removeEventListener(
                ...
                if (TP.isCallable(loadedHandler)) {
                    loadedHandler(newLinkElement);
                }

                //  Resolve the Promise with the script element that the script
                //  is loaded into.
                resolver(newLinkElement);
            };

            newLinkElement.addEventListener('load', successHandler, false);

            successHandler = function(err) {
            ...
            newLinkElement.addEventListener('error', errorHandler, false);

            //  We don't have to worry about reassignment of newLinkElement to
            //  the return value of this method since we know we created it in
            //  targetDoc.
...
function(cssElements, targetDoc, shouldSignal) {
    ...
    for (i = 0; i < cssElements.getSize(); i++) {
        ...
        if (localName === 'link') {
            //  Note that, since we're not interested in the return value here,
            //  we don't await on the value.
...
async function(anElement, targetDoc, inlineRuleText, onlyIfAbsent, shouldSignal) {
...
function(anElement) {
    ...
    if (TP.notEmpty(hrefsToAdd)) {
        ...
        for (i = 0; i < len; i++) {
            ...
            //  signaling. Also note that, since we're not interested in the
            //  return value here, we don't await on the value.
