...
function(aRequest) {
    ...
    //  If we are a URI, then try to extract our String value so that we can
    //  report it below. Otherwise report it as 'unknown'.
    if (TP.isKindOf(this, TP.uri.URI)) {
    ...
    if (async && this.getType().isSyncOnly()) {
        TP.ifDebug() ?
            TP.debug('Overriding async request for sync-only URI: ' + uri) : 0;
    ...
    if (!async && this.getType().isAsyncOnly()) {
        ...
        TP.ifDebug() ?
            TP.debug('Overriding sync request for async-only URI: ' + uri) : 0;
