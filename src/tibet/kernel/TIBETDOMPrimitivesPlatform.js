...
TP.hc(
    ...
    function(aNode) {
         ...
         * @synopsis Normalizes adjacent Text nodes on the supplied Node and its
         *     descendants.
         * @description On Webkit we supply our own implementation of this
         *     method because there were problems with the regular 'normalize()'
         *     method improperly merging adjacent Text and CDATA Section nodes.
         *     See: https://bugs.webkit.org/show_bug.cgi?id=33191
        ...
        //  NB: The 'childNodes' NodeList here is live, which is what we
        //  want

        for (i = 0; i < aNode.childNodes.length; i++) {
            currentChild = aNode.childNodes[i];

            if (TP.isTextNode(currentChild)) {
                if (TP.isTextNode(previousChild =
                                    currentChild.previousSibling)) {
                    previousChild.data += currentChild.data;
                    aNode.removeChild(currentChild);

                    //  Make sure to decrement the counter by 1, since we
                    //  just removed a child and the 'childNodes' NodeList
                    //  is live.
                    i--;
                }
            } else if (TP.isElement(currentChild)) {
                TP.nodeNormalize(currentChild);
            }
        }
    ...
    function(aNode) {
        ...
        //  We wrapped the regular 'normalize()' call because Webkit had
        //  problems and we overlay it in the Webkit file.
