...
TP.sys.defineMethod('fetchAssetsInto',
function(assetInfoList, aDocument) {

    /**
     * @method fetchAssetsInfo
     * @summary Grabs the 'uri' of each entry in the assetInfoList and loads it
     *     into the supplied document's context. Note that this call is done in
     *     a asynchronous fashion, returning a Promise that will resolve when
     *     *all* assets have been loaded..
     * @param {TP.core.Hash[]} assetInfoList A list of TP.core.Hashes that each
     *     contain an asset 'uri', an asset 'type' and an optional set of
     *     'elemAttrs' (attributes to put onto the 'script', 'link', etc.
     *     element that will be created)..
     * @param {Document} [aDocument=document] The document to add the asset to.
     *     This defaults to the code frame document.
     * @returns {Promise} A promise which resolves based on success.
     */

    var promises;

    promises = assetInfoList.map(
                function(anAssetInfo) {
                    var assetType,
                        assetURI,
                        elemAttrs;

                    assetURI = anAssetInfo.at('uri');
                    if (TP.isEmpty(assetURI)) {
                        //  TODO: Warn
                        return Promise.resolve();
                    }

                    elemAttrs = anAssetInfo.at('elemAttrs');

                    assetType = anAssetInfo.at('type');
                    switch (assetType) {
                        case 'script':
                            return TP.sys.fetchScriptInto(
                                    ...
                                    null,
                                    elemAttrs,
                                    anAssetInfo.at('isECMAModule'));
                        case 'style':
                            //  NB: null for the 'beforeNode' and false for
                            //  signaling.
                            return TP.documentAddCSSLinkElement(
                                    aDocument,
                                    TP.uriExpandPath(assetURI),
                                    null,
                                    false).then(
                                    function(linkElem) {
                                        //  If additional script element
                                        //  attributes were supplied, then put
                                        //  them on the script element.
                                        if (TP.notEmpty(elemAttrs)) {
    ...
    //  result objects (script, style, etc. elements or ECMA modules) that have
    //  their specifier as the key and the result object as the value.
    return Promise.all(promises).then(
        function(response) {
            ...
            response.forEach(
                function(aResult, index) {
                    resultObjects.atPut(
                            assetInfoList.at(index).at('uri'), aResult);
                });
    ...
    if (isECMAModule) {
        ...
        newPromise = Promise.construct(
            function(resolver, rejector) {
                return TP.sys.importModule(targetLoc, win, true).then(
                    function(moduleObj) {
...
function(aSpecifier, loadRealm, forceNativeImport) {
     ...
     *     the module location.
     * @param {Window} [loadRealm=window] The realm (i.e. Window) to load the
     *     module into. This defaults to the code frame window.
    ...
    if (TP.isValid(loadRealm) && TP.isCallable(loadRealm.importModule)) {
        return loadRealm.importModule(finalUrl);
    }
...
function(aSpecifierList, loadRealm, forceNativeImport) {
    ...
    promises = aSpecifierList.map(
                function(specifier) {
                    return TP.sys.importModule(
                                    specifier, loadRealm, forceNativeImport);
                });
...
async function(aTypeName, aRealmID) {
    ...
    /**
     * @method getOtherRealmAssets
     * @summary Returns a list of 'other realm' assets that are defined in the
     *     manifest system for the type matching the supplied type
     * @param {String} aTypeName The type name to retrieve other realm asset
     *     entries for.
     * @param {String} aRealmID The ID of the realm to retrieve the assets for -
     *     something like 'uicanvas'.
     * @exception TP.sig.InvalidType Raised when a valid type cannot be resolved
     *     from the type name provided to the method.
     * @returns {String[]|null} A list of the 'other realm' assets for the named
     *     type.
     */
    ...
    for (asset of assets) {
        if (TP.elementGetAttribute(asset, 'realm', true) === aRealmID) {
            path = TP.ifEmpty(
                    ...
                    TP.elementGetAttribute(asset, 'href'));
