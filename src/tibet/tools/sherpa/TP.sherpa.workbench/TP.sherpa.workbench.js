...
function(aSignal) {
    ...
    stores.getKeys().forEach(
            function(loc) {
                ...
                httpObj = TP.httpGet(loc, TP.request('async', false));
                ...
                contentURI = TP.uc(loc);

                //  Save a diff patch between the new content and the
                //  server-side content and push it to the server.
                successfulPatch = contentURI.saveDiffPatchAgainst(
                                                    newContent,
                                                    currentContent);

                //  If the patch was successful, then we can now use it as the
                //  content in the URI.
                if (successfulPatch) {
                    contentURI.setResource(newContent);
                }
            });
