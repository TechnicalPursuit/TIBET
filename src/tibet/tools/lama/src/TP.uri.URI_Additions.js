...
function(options) {
    ...
    //  If not, then possibly reset the content type to be that for a Lama
    //  urieditor.
    if (!tabHasValue) {
...
function(options) {
    ...
    if (tabHasValue) {
        TP.byId('LamaConsoleTabbar', TP.sys.getUIRoot()).setValue(loc);
    ...
    uriEditorTPElem = TP.lama.urieditor.getResourceElement(
                            'template',
                            TP.ietf.mime.XHTML);
