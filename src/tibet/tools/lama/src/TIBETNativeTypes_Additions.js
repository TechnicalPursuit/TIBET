...
//  possible to add traits (like TP.lama.ToolAPI) to builtin types.
//  ------------------------------------------------------------------------
...
function(options) {
    ...
    tabHasValue = TP.byId('LamaConsole', TP.sys.getUIRoot()).hasTabForValue(
                                                                displayName);
...
function(options) {
    ...
    options.atPut(TP.ATTR + '_childtype', 'lama:methodeditor');
...
function(options) {
    ...
    if (tabHasValue) {
        TP.byId('LamaConsoleTabbar', TP.sys.getUIRoot()).setValue(displayName);
        TP.byId('LamaConsolePanelbox', TP.sys.getUIRoot()).
    ...
    methodEditorTPElem = TP.lama.methodeditor.getResourceElement(
                            'template',
                            TP.ietf.mime.XHTML);
