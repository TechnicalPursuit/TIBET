...
/**
 * @type {TP.lama.console}
 */

//  ------------------------------------------------------------------------

TP.lama.Element.defineSubtype('lama:console');
...
TP.lama.console.Type.defineConstant('DEFAULT_PROMPT', '&#160;&#187;');
...
TP.lama.console.Inst.defineAttribute('$minEditorHeight');

//  the minimum height of the console drawer.
TP.lama.console.Inst.defineAttribute('$minDrawerHeight');

//  the Function that will stop ACE from processing events.
TP.lama.console.Inst.defineAttribute('$aceEventStopper');

//  should we move the cursor?
TP.lama.console.Inst.defineAttribute('allowMouseCursorMovement');

//  should IO be concealed? this is used to simulate "password" mode
TP.lama.console.Inst.defineAttribute('conceal', false);

//  is the command line currently concealed from view?
TP.lama.console.Inst.defineAttribute('concealedInput');

TP.lama.console.Inst.defineAttribute('consoleInput',
    TP.cpc('xctrls|codeeditor#LamaConsoleInput',
        TP.hc('shouldCollapse', true)));

TP.lama.console.Inst.defineAttribute('consoleOutput');

//  the number of 'new' items since we evaluated last
TP.lama.console.Inst.defineAttribute('newOutputCount');

//  a timer that will flip the status readout back to mouse coordinates after a
//  period of time.
TP.lama.console.Inst.defineAttribute('statusReadoutTimer');

//  ------------------------------------------------------------------------
//  Instance Methods
//  ------------------------------------------------------------------------

TP.lama.console.Inst.defineMethod('createNewConsoleEditorFor',
function(tabValue, tabLabel) {
    ...
    if (TP.isURIString(tabValue)) {
        ...
        editorTPElem = TP.lama.urieditor.getResourceElement(
                                'template',
                                TP.ietf.mime.XHTML);

        toolbarTPElem = TP.lama.uriEditorToolbarContent.getResourceElement(
                                'template',
                                TP.ietf.mime.XHTML);
        ...
        editorTPElem = TP.lama.methodeditor.getResourceElement(
                                'template',
                                TP.ietf.mime.XHTML);

        toolbarTPElem = TP.lama.methodEditorToolbarContent.getResourceElement(
                                'template',
                                TP.ietf.mime.XHTML);
...
TP.lama.console.Inst.defineMethod('getTabCount',
function() {
    ...
    tabsURI = TP.uc('urn:tibet:lama_south_drawer_tabs');
...
TP.lama.console.Inst.defineMethod('hasTabForValue',
function(aValue) {
...
TP.lama.console.Inst.defineMethod('makeNewConsoleTabPanel',
function(tabValue, tabLabel, shouldCreateProfileEntry) {
    ...
    //  Grab the Lama console's panel box and add a new panel, supplying the
    //  value we are using to tie the two together.
    panelBox = TP.byId('LamaConsolePanelbox', this.getNativeDocument());
...
TP.lama.console.Inst.defineMethod('removeConsoleTabPanel',
function(tabValue) {
    ...
    tabbar = TP.byId('LamaConsoleTabbar', this.getNativeDocument());
    ...
    inspector = TP.byId('LamaInspector', this.getNativeDocument());
...
TP.lama.console.Inst.defineMethod('setup',
function() {
    ...
    spaceHandler =
        function() {
            ...
            if (mode === 'none') {
                ...
                consoleOutput.removeAttribute('viewall');
                ...
                if (consoleOutput.hasAttribute('viewall')) {
                    consoleOutput.removeAttribute('viewall');
                } else {
                    consoleOutput.setAttribute('viewall', 'true');
                }
    ...
    consoleOutputTPElem = TP.byId('LamaConsoleOutput',
    ...
    TP.uc('urn:tibet:lama_south_drawer_tabs').setResource(data);

    //  Set the selection hash for the console tabs.
    data = TP.hc('selection', 'TSH');
    TP.uc('urn:tibet:lama_current_south_drawer_tab').setResource(data);
...
TP.lama.console.Inst.defineHandler('PclassClosedChange',
function(aSignal) {
...
TP.lama.console.Inst.defineHandler('PclassHiddenChange',
function(aSignal) {
...
TP.lama.console.Inst.defineHandler('RemoveConsoleTab',
function(aSignal) {
...
TP.lama.console.Inst.defineHandler('LamaReady',
function(aSignal) {
...
TP.lama.console.Inst.defineHandler('ValueChange',
function(aSignal) {
    ...
    if (aSignal.getOrigin() === TP.uc('urn:tibet:lama_consoletabs')) {
        ...
        if (TP.notEmpty(editorTabEntries)) {
            ...
            currentTabURI = TP.uc('urn:tibet:lama_current_south_drawer_tab');
...
TP.lama.console.Inst.defineHandler('AddSnippet',
function(aSignal) {
...
TP.lama.console.Inst.defineMethod('render',
function() {
...
TP.lama.console.Inst.defineMethod('setAllowMouseCursorMovement',
...
TP.lama.console.Inst.defineMethod('setAttrHidden',
function(beHidden) {
...
TP.lama.console.Inst.defineMethod('setupConsoleService',
function() {
    ...
    TP.lama.ConsoleService.construct(
        null,
...
TP.lama.console.Inst.defineMethod('clear',
function() {
...
TP.lama.console.Inst.defineMethod('clearAllContent',
function() {
...
TP.lama.console.Inst.defineMethod('setIndicatorAttribute',
...
TP.lama.console.Inst.defineMethod('toggleIndicatorVisibility',
...
TP.lama.console.Inst.defineMethod('toggleObservations',
function(shouldObserve) {
    ...
    tabDataURI = TP.uc('urn:tibet:lama_consoletabs');
    tabSelectionURI = TP.uc('urn:tibet:lama_current_south_drawer_tab#tibet(selection)');
...
TP.lama.console.Inst.defineMethod('setPrompt',
...
TP.lama.console.Inst.defineMethod('clearStatus',
...
TP.lama.console.Inst.defineMethod('updateStatus',
    ...
    if (statID === 'keyboardInfo' || statID === TP.ALL) {
        ...
        if (TP.canInvoke(aSignal, 'getEvent') &&
            ...
            if (TP.isKindOf(aSignal, TP.sig.DOMKeyUp)) {
                ...
                setTimeout(
                    timer,
                    TP.sys.cfg('lama.readout_mouse_reset_time', 1000));
...
TP.lama.console.Inst.defineMethod('adjustInputSize',
...
TP.lama.console.Inst.defineMethod('clearInput',
function() {
...
TP.lama.console.Inst.defineMethod('focusInput',
function(select) {
...
TP.lama.console.Inst.defineMethod('getInputContent',
function() {
...
TP.lama.console.Inst.defineMethod('setInputCursorToEnd',
function() {
...
TP.lama.console.Inst.defineMethod('setInputContent',
...
TP.lama.console.Inst.defineMethod('decreaseOutputDisplayMode',
function() {
...
TP.lama.console.Inst.defineMethod('getOutputDisplayMode',
function() {
...
TP.lama.console.Inst.defineMethod('increaseOutputDisplayMode',
function() {
...
TP.lama.console.Inst.defineMethod('setOutputDisplayMode',
function(displayModeVal) {
    ...
    //  re-populated by machinery in the TP.lama.consoleoutput type as
    //  necessary.
    consoleOutput.removeAttribute('exposed');
...
TP.lama.console.Inst.defineMethod('shouldCoalesceLogMessages',
function() {
...
TP.lama.console.Inst.defineMethod('createOutputItem',
...
TP.lama.console.Inst.defineMethod('updateOutputItem',
function(uniqueID, dataRecord) {
...
TP.lama.console.Inst.defineMethod('scrollOutputToEnd',
function() {
...
TP.lama.console.Inst.defineMethod('setNewOutputCount',
