...
/**
 * @type {TP.lama.formBuilderAssistant}
 */

//  ------------------------------------------------------------------------

TP.tag.CustomTag.defineSubtype('lama.formBuilderAssistant');

//  Note how this property is TYPE_LOCAL, by design.
TP.lama.formBuilderAssistant.defineAttribute('themeURI', TP.NO_RESULT);

//  ------------------------------------------------------------------------
//  Instance Attributes
//  ------------------------------------------------------------------------

TP.lama.formBuilderAssistant.Inst.defineAttribute('head',
    TP.cpc('> .head', TP.hc('shouldCollapse', true)));

TP.lama.formBuilderAssistant.Inst.defineAttribute('body',
    TP.cpc('> .body', TP.hc('shouldCollapse', true)));

TP.lama.formBuilderAssistant.Inst.defineAttribute('foot',
    TP.cpc('> .foot', TP.hc('shouldCollapse', true)));

TP.lama.formBuilderAssistant.Inst.defineAttribute('data');

//  ------------------------------------------------------------------------
//  Type Methods
//  ------------------------------------------------------------------------

TP.lama.formBuilderAssistant.Type.defineMethod('showAssistant',
function(assistantData) {
     ...
     * @returns {TP.meta.lama.formBuilderAssistant} The receiver.
     */

    var assistantContentTPElem,
        dialogPromise;

    //  Grab the TP.lama.formBuilderAssistant type's template.
    assistantContentTPElem =
        TP.lama.formBuilderAssistant.getResourceElement(
                        'template',
                        TP.ietf.mime.XHTML);
...
TP.lama.formBuilderAssistant.Inst.defineMethod('generatePathData',
function(anElement) {
     ...
     * @returns {TP.lama.formBuilderAssistant} The receiver.
     */
...
TP.lama.formBuilderAssistant.Inst.defineHandler('DialogCancel',
function(anObject) {
    ...
    //  on our way out.
    modelURI = TP.uc('urn:tibet:formBuilderAssistant_source');
...
TP.lama.formBuilderAssistant.Inst.defineHandler('DialogOk',
function(anObject) {
    ...
    result = TP.uc('urn:tibet:formBuilderAssistant_source').
    ...
    if (TP.isKindOf(suppliedURI, TP.uri.URL)) {
        ...
        //  Compute a URI to use for local temporary storage from the supplied
        //  local storage ID.
        localID = suppliedData.at('localStorageID');
        localLoc = 'urn:tibet:' + localID;

        remoteLoc = suppliedURI.getLocation();

        //  ---

        //  Set the top-level scope to the local temporary storage
        ...
        //  Non-URL URIs just use the supplied URI as the 'local temporary
        //  storage'.

        //  Use the supplied URI as the URI for local temporary storage.
        localLoc = suppliedURI.getLocation();
    ...
    if (TP.isEmpty(schemaFile)) {
        TP.bySystemId('Lama').insertElementIntoCanvas(
            ...
            true,
            false,
            function(insertedTPElem) {
                //  Refresh the property sheet element, since we just inserted
                //  it and its got bindings.
                insertedTPElem.refresh();
            });
    } else {
        ...
        if (!TP.isURI(schemaURI)) {
            TP.bySystemId('Lama').insertElementIntoCanvas(
                ...
                false,
                function(insertedTPElem) {
                    //  Refresh the property sheet element, since we just
                    //  inserted it and its got bindings.
                    insertedTPElem.refresh();
                });
        } else {
                ...
                'RequestSucceeded',
                    function(aResponse) {
                        ...
                        TP.bySystemId('Lama').insertElementIntoCanvas(
                            ...
                            false,
                            function(insertedTPElem) {
                                //  Refresh the property sheet element, since we
                                //  just inserted it and its got bindings.
                                insertedTPElem.refresh();
                            });
                    });
...
TP.lama.formBuilderAssistant.Inst.defineMethod('setData',
function(anObj) {
    ...
    definitionName = 'TIBET_Type_Doc_' + TP.genID().replace('$', '_');

    //  Build a JSON Schema from the POJO data and with the ID of the JSON
    ...
    breadcrumbTPElem = TP.byId(
                        'formBuilderAssistant_InsertionBreadcrumb',
                        this.getNativeNode());
